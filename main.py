...
# =====================================================================
# ГЕНЕРАТОР НАСТОЯЩИХ ЛАБИРИНТОВ (Recursive Backtracker)
# =====================================================================
def generate_static_level(seed_val, num_enemies, num_coins, level_index):
    random.seed(seed_val)
    COLS, ROWS = 26, 15
    # Изначально всё заполнено стенами
    grid = [[1 for _ in range(COLS)] for _ in range(ROWS)]
    
    # Алгоритм прокладывания лабиринта
    start_c, start_r = 1, 1
    ...
    while stack:
        c, r = stack[-1]
        neighbors = []
        # Проверяем клетки через 1 стену
    ...
    # Фикс для правого края (24-я колонка)
    ...
    def creates_2x2_empty(test_r, test_c):
        """Проверяет, не образуется ли пустая комната 2x2 при удалении стены"""
    ...
    # Снос случайных стен для создания кольцевых путей
    loop_chance = 0.25 + (level_index * 0.01) # 25-40% шанс пробить стену
    for r in range(1, ROWS - 1):
        for c in range(1, COLS - 1):
            if grid[r][c] == 1:
                ...
                # Пробиваем только если это прямая стена между двумя коридорами
...
# Таблица прогрессии уровней: (кол-во врагов, кол-во монет)
level_configs = [
    (1, 5), (1, 10), (1, 15),       # Уровни 1-3
    (2, 10), (2, 15), (2, 20),      # Уровни 4-6
    (3, 10), (3, 15), (3, 20),      # Уровни 7-9
    (4, 10), (4, 15), (4, 20),      # Уровни 10-12
    (5, 15), (5, 20), (5, 25)       # Уровни 13-15
]

# Генерируем все 15 уровней перед стартом игры
levels = []
...
def resolve_x(px, py, dx):
    ...
    for wall in get_nearby_walls(new_px, py):
        if hb.colliderect(wall):
            if dx > 0: return wall.left - OFFSET_X - HITBOX_W
            else: return wall.right - OFFSET_X
    return new_px
...
def resolve_y(px, py, dy):
    ...
    for wall in get_nearby_walls(px, new_py):
        if hb.colliderect(wall):
            if dy > 0: return wall.top - OFFSET_Y - HITBOX_H
            else: return wall.bottom - OFFSET_Y
    return new_py

def draw_button(surface, text, x, y, w, h, img, fnt):
...
while running:
    ...
    if game_state == STATE_MENU:
        ...
        btn_w, btn_h = 240, 70
        btn_x = (WIDTH - btn_w) // 2
        start_btn = draw_button(screen, "START", btn_x, 300, btn_w, btn_h, btn_img, font)
        exit_btn = draw_button(screen, "EXIT", btn_x, 400, btn_w, btn_h, btn_img, font)
    ...
    # ---------- ИГРА ----------
        ...
        # 1. Отрисовка уровня на виртуальную поверхность (для центрирования)
        game_surf = pygame.Surface((COLS * TILE, ROWS * TILE))
        game_surf.blit(static_bg, (0, 0))
        if start_pos: game_surf.blit(start_img, (start_pos[0] * TILE, start_pos[1] * TILE))
        if finish_pos: game_surf.blit(finish_img, (finish_pos[0] * TILE, finish_pos[1] * TILE))
    ...
    # ---------- ЭКРАН СМЕРТИ ----------
        ...
        game_surf = pygame.Surface((COLS * TILE, ROWS * TILE))
        game_surf.blit(static_bg, (0, 0))
        for col, row in coin_cells: game_surf.blit(coin_img, (col * TILE, row * TILE))
        for devil in devils: devil.draw(game_surf)
        game_surf.blit(player_img, (int(player_x), int(player_y)))
